...
<body>
    ...
    <?php
			if(isset($_GET['aksi']) == 'delete'){
				...
				$cek = mysqli_query($con, "SELECT * FROM tbl_preguntas WHERE ID_Pregunta='$nik'");
				if(mysqli_num_rows($cek) == 0){
					echo '<div class="alert alert-info alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button> No se encontraron datos.</div>';
				}else{
					$delete = mysqli_query($con, "DELETE FROM tbl_preguntas WHERE ID_Pregunta='$nik'");
					if($delete){
						$id_usuario= $_SESSION['ID_Usuario'];
							$insert_bitacora = mysqli_query($con, "INSERT INTO tbl_bitacora_evento (id_usuario,id_objeto,Accion,Descripcion)
                            VALUES ('$id_usuario',17,'Delete','SE ELIMINÓ UNA PREGUNTA')") or die(mysqli_error());
	 ...
	 $sql=("SELECT * FROM tbl_permisos WHERE id_objeto=13 AND id_rol=$rolUsuario");
		...
		<form class="form-inline my-2 my-lg-0 float-left">
			<a href="agregarpregunta.php" class="genric-btn info circle">Agregar</a>
		</form>
		<form class="form-inline my-2 my-lg-0 float-left">
			<a class="genric-btn btn-danger circle" onclick="pdfDinamico(this)" id="pdf_preguntas_secretas" href="#">
