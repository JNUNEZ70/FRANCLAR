...
function Header()
{
    ...
    $this->Cell(50);
    // Título
    $this->Cell(100,10,utf8_decode('REPORTE DE PARÁMETROS DE SEGURIDAD'),0,0,'C');
    // Salto de línea
    $this->Ln(20);
}
...
if ($_GET['perl']) {
    ...
    $carg="SELECT * FROM tbl_parametros WHERE id_parametro LIKE '%".$buscar."%' OR parametro LIKE '%".$buscar."%' OR valor LIKE '%".$buscar."%'";
...
$pdf->Cell(150, 10, utf8_decode('Parámetro'), 1, 0, 'C', 1);
$pdf->Cell(20, 10, 'Valor', 1, 1, 'C', 1);

while($row = mysqli_fetch_assoc($consulta)){
    $pdf->Cell(20, 10, $row['id_parametro'], 1, 0, 'C', 0);
    $pdf->Cell(150, 10, utf8_decode($row['parametro']), 1, 0, 'C', 0);
    $pdf->Cell(20, 10, $row['valor'], 1, 1, 'C', 0);
}
